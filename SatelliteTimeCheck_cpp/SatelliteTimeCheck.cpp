...
void SatelliteTimeCheck_3_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void SatelliteTimeCheck_3_run(uint ctx_index,Circom_CalcWit* ctx);
Circom_TemplateFunction _functionTable[4] = { 
Num2Bits_0_run,
LessThan_1_run,
GreaterThan_2_run,
SatelliteTimeCheck_3_run };
...
uint get_total_signal_no() {return 526;}

uint get_number_of_components() {return 6;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 520;}
...
ctx->componentMemory[coffset].templateName = "SatelliteTimeCheck";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 3],&signalValues[mySignalStart + 0]); // line circom 17
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[1],&signalValues[mySignalStart + 5],&signalValues[mySignalStart + 1]); // line circom 20
Fr_sub(&expaux[0],&expaux[1],&signalValues[mySignalStart + 4]); // line circom 20
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_add(&expaux[0],&signalValues[mySignalStart + 6],&signalValues[mySignalStart + 2]); // line circom 24
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
PFrElement aux_dest = &ctx->signalValues[ctx->componentMemory[mySubcomponents[cmp_index_ref]].signalStart + 1];
// load src
Fr_sub(&expaux[0],&signalValues[mySignalStart + 6],&signalValues[mySignalStart + 2]); // line circom 28
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
// no need to run sub component
...
{{
Fr_eq(&expaux[0],&signalValues[mySignalStart + 7],&circuitConstants[2]); // line circom 34
}}
...
SatelliteTimeCheck_3_create(1,0,ctx,"main",0);
SatelliteTimeCheck_3_run(0,ctx);
}
